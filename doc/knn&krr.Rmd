...
# extract_pred_rating <- function(test_set, pred){
#   pred_rating <- pred[as.character(test_set[2]), as.character(test_set[1])]
#   #as.character(test_set[2]) - movie id
#   #as.character(test_set[1]) - user id
#   return(pred_rating)
# }
# #extract predicted rating
# str(pred_rating) # pred_rating sequential U and I
# pred_test_rating <- apply(data_test, 1, extract_pred_rating, pred_rating)
# 
...
user=as.character(1:U)
KRR_pred=matrix(0,9724,length(user))
#str(pred_rating) # 9724* 610
for (i in 1:length(user)){
  y=pred_rating[,user[1]] # 9742*1
  # a=km%*%y
  # #str(a) #9724*1
  # w=t(X)%*%a
  # #str(w)
  K=exp(2*(t(X)%*%X-1))
  lambda=1
  w=X%*%solve(K+lambda*diag(10))%*%t(y)
  KRR_pred[,i]=X%*%w
}
