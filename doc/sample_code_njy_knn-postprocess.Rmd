...
KNN_pred_rating <- function(test_set, pred,K){
  ...
  c_idx <- colnames(result$q) == as.character(m)
  vm <- result$q[,c_idx]
  V_M <- result$q[,-c_idx]
  
  s <- apply(V_M,2,function(x) vm%*%x/sqrt(sum(x^2))/sqrt(sum(vm^2)))
  
  mid_ch <- c()
  for (i in 1:K) {
    m_idx <- colnames(V_M)[which(s == sort(s,decreasing = TRUE)[i])]
    mid_ch <- c(mid_ch,m_idx)
  }
  
  pred_rating.um <- pred[as.character(mid_ch), as.character(u)]
  return(mean(pred_rating.um))
}
...
pred_test_rating <- apply(data_test, 1, KNN_pred_rating, pred_rating,10)
