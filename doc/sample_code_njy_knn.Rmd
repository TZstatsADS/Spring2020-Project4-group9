...
KNN_pred_rating <- function(test_set, pred,K){
  ...
  m_idx <- colnames(result$q) == as.character(m)
  #u_idx <- colnames(result$p) == as.character(u)
  Q <- rbind(result$q,as.vector(pred[,as.character(u)]))
  Q <- t(Q)
  colnames(Q) <- c((paste("f",c(1:10),sep = "")),"rt_p")
  Q <- as.data.frame(Q)
  
  vm <- Q[m_idx,]
  V_M <- Q[-m_idx,]
  
  k.te <- kknn(rt_p~.,train = V_M,test = vm,k=K,distance = 2,kernel = "cos")
  
  
  pred_rating.um <- k.te$fitted.values
  return(pred_rating.um)
}


rmse_k <- c()

k <- c(1,2,3,5,10,15,20)
for (i in 1:length(k)) {
...
KNN_pred_rating <- function(test_set, pred,K){
  ...
  mid_ch <- c()
  for (i in 1:K) {
    m_idx <- colnames(m.s)[which(m.s[as.character(m),] == sort(m.s[as.character(m),],decreasing = TRUE)[i+1])]
...
KNN_pred_rating <- function(test_set, pred,K){
  ...
  pred_rating.um <- pred[mid_ch, as.character(u)]
  pred_rating.um.wgt <- m.s[as.character(m),as.character(mid_ch)]
  
  pred_rating <- sum(pred_rating.um.wgt*pred_rating.um)/sum(pred_rating.um.wgt)
  return(pred_rating)
}
