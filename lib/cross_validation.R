...
cv.function.als <- function(dat_train, K, f, lambda){
  ...
  set.seed(0)
  s <- sample(rep(1:K, c(rep(n.fold, K-1), n-(K-1)*n.fold)))  
  train_rmse <- matrix(NA, ncol = 100,nrow = K)
  test_rmse <- matrix(NA, ncol = 100, nrow = K)
  
  for (i in 1:K){
    train.data <- dat_train[s != i,]
    test.data <- dat_train[s == i,]
    
    result <- als(f = f, lambda = lambda, max.iter = 100, stopping.thres = 0.001, data = dat_train, train = train.data, test = test.data)
